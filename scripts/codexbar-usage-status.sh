...
refresh_cache() {
  ...
  set +e
  fetch_out="$(codexbar --provider claude --source oauth --format json --json-only --web-timeout "$WEB_TIMEOUT_SECONDS" 2>"$stderr_file")"
  local fetch_status=$?
  set -e
  ...
  if (( fetch_status != 0 )); then
    if [[ "$fetch_out" == *"Unknown option --json-only"* || "$fetch_err" == *"Unknown option --json-only"* ]]; then
      ...
      set +e
      fetch_out="$(codexbar --provider claude --source oauth --format json --web-timeout "$WEB_TIMEOUT_SECONDS" 2>"$stderr_file")"
      fetch_status=$?
      set -e
      fetch_err="$(cat "$stderr_file" 2>/dev/null || true)"
    fi
